...
import { CreateTipoUniformeDto } from './dto/create-tipo-uniforme.dto';
...
@Controller('tipos-uniforme')
@UseGuards(JwtAuthGuard, PermissionsGuard)
export class TiposUniformeController {
  constructor(private readonly service: TiposUniformeService) {}

  @Post()
  @RequirePermissions('inventarios:configurar')
  create(
    @CurrentUser('empresa_id') empresaId: number,
    @Body() dto: CreateTipoUniformeDto,
  ) {
  ...
  update(
    @Param('id', ParseIntPipe) id: number,
    @CurrentUser('empresa_id') empresaId: number,
    @Body() dto: UpdateTipoUniformeDto,
  ) {
