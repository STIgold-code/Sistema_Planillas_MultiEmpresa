...
export class VacantesController {
  ...
  findAll(@CurrentUser() user: any, @Query() filters: FilterVacanteDto) {
  ...
  create(@Body() dto: CreateVacanteDto, @CurrentUser() user: any) {
  ...
  @Patch(':id/publicar')
  @RequirePermissions('seleccion:editar')
  publicar(@Param('id', ParseIntPipe) id: number, @CurrentUser() user: any) {
    return this.vacantesService.cambiarEstado(id, user.empresa_id, 'PUBLICADA');
  }

  @Patch(':id/cerrar')
  @RequirePermissions('seleccion:editar')
  cerrar(@Param('id', ParseIntPipe) id: number, @CurrentUser() user: any) {
